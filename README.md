...
- **Annotate** — From the pencil dialog on the song list or song viewer. Images and placeholders are converted to a title-based PDF first (same archive row, original image deleted if nothing else uses it). Then **Xodo** opens when installed; otherwise a dialog offers **Install Xodo** or **Use another app**. Markup is written back onto the song PDF when the editor saves our file — no second import. **↓ Share** remains a copy-out. Remote play does not offer Annotate.
...
9. **Settings** — Main tabs **☰** → **Settings** → set the remote code and OpenAI API key → **Save**. **Adjust page after camera** and **Hide song share button** are switches (saved immediately). **Check for updates** anytime from the same screen.
