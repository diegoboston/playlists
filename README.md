...
2. **Start** — Open a playlist → tap **Wi‑Fi** (or the main-tab shortcut). Pick **Cloudflare tunnel** for internet (`https://….trycloudflare.com/` — no port) or **LAN only** for same-Wi‑Fi (`http://phone-ip:code/`). The phone starts the HTTP server and shows a dialog with the URL (clickable link, **▼** chevron to show a QR code, and **Open in browser**). A **foreground notification** with **Stop** also appears; it does not include the URL or code.
