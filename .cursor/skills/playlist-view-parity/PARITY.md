...
| Playlist name in header | Playlist accent color | Plain `#title` text | OK — color is local-only |
| Rename / duplicate / color / delete playlist | Toolbar on detail screen | Not available | **Intentional** (local only) |
...
| Endpoint | Purpose |
| -------- | ------- |
| `GET /api/state` | Playback position + song list for `play.html` |
| `GET /api/playlist` | Full entries for `edit.html` |
| `POST /api/navigate` | prev/next song/page |
| `GET /api/media` | Image/PDF bytes for current page |
| `POST /api/reorder` | Persist entry order |
| `POST /api/remove` | Remove entry from playlist |
| `POST /api/add` | Add archive song to playlist |
| `GET /api/songs/search` | Archive search for add dialog |
| `POST /api/upload` | Add song file + metadata (remote play) |
